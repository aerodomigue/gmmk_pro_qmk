/* Copyright 2025 aerodomigue <43935382+aerodomigue@users.noreply.github.com>
...
enum custom_keycodes {
    FN_SLEP,          // Cette touche remplace l’Échap : macro si MY_FN est appuyé, sinon envoie KC_ESC
};
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    ...
    if (get_highest_layer(layer_state) > 0) {
        ...
        switch (curr_layer) {
            ...
            //NKRO toggle key - show current state
            if (keymap_config.nkro) {
                rgb_matrix_set_color(LED_F13, RGB_GREEN);  // Green when NKRO is enabled
            } else {
                rgb_matrix_set_color(LED_F13, RGB_ORANGE); // Orange when NKRO is disabled (6KRO mode)
            }
